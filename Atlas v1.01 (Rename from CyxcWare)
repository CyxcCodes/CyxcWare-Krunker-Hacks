...
// @icon         https://cdn-icons-png.flaticon.com/512/10305/10305383.png
// @grant        none
...
waitForThreeJs().then((THREE) => {
    ...
    function handleShoot(event) {
        if (event.button === 0 && settings.aimbotEnabled && settings.silentAim && targetPlayer && silentAimPosition) {
            ...
            x.consoleLog('Silent aim applied: please shot at screen coordinates', screenPos);
        }
    }
    ...
    function createGUI() {
        const guiEl = fromHtml(`<div class="zui">
            <div class="zui-item zui-header">
                <span>[/] Controls and Userscript:</span>
        ...
        headerEl.onclick = function () {
            ...
            headerStatusEl.innerText = isHidden ? '[close script]' : '[open script]';
        };
        ...
        contentEl.appendChild(fromHtml(`<div class="zui-item text">
            <span>If your reading this then the script currently works!</span>
        </div>`));
